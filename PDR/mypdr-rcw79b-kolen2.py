...
m2 = Measurement(data=6.3,uncertainty = StdDevUncertainty([np.sqrt((6.3*0.2)**2+1.0**2)]),
                 identifier="CI_609",restfreq="492.160651 GHz",unit="K km/s")
m3 = Measurement(data=21.8,uncertainty = StdDevUncertainty([np.sqrt((21.8*0.2)**2+1.2**2)]),
...
m5 = Measurement(data=16.5,uncertainty = StdDevUncertainty([np.sqrt((16.5*0.1)**2+0.6**2)]),
                 identifier="CO_10",restfreq="115.2712018 GHz",unit="K km/s")

m7 = Measurement(data=6.1,uncertainty = StdDevUncertainty([np.sqrt((6.1*0.1)**2+0.4**2)]),
...
fig.savefig("RCW79B-corner-koeln-non.pdf",bbox_inches='tight',pad_inches=0)
