...
m2 = Measurement(data=8.8,uncertainty = StdDevUncertainty([np.sqrt((8.8*0.2)**2+1.4**2)]),
                 identifier="CI_609",restfreq="492.160651 GHz",unit="K km/s")
m3 = Measurement(data=49.9,uncertainty = StdDevUncertainty([np.sqrt((49.9*0.2)**2+3.4**2)]),
                 identifier="CO_43",restfreq="461.04077 GHz", unit="K km/s")
m4 = Measurement(data=26.5,uncertainty = StdDevUncertainty([2.7]),
                 identifier="CII_158",restfreq="1900.5369 GHz",unit="K km/s")


m5 = Measurement(data=35.6,uncertainty = StdDevUncertainty([np.sqrt((35.6*0.1)**2+0.6**2)]),
                 identifier="CO_10",restfreq="115.2712018 GHz",unit="K km/s")

m7 = Measurement(data=11.9,uncertainty = StdDevUncertainty([np.sqrt((11.9*0.1)**2+0.2**2)]),
                 identifier="13CO_10",restfreq="110.2013541 GHz",unit="K km/s")


a = [m2,m3,m4,m5,m7]

ms = ModelSet("kt2020",medium="non-clumpy",mass=1,z=1)
...
fig = corner.corner(rescopy, bins=20,
                    labels=[r"$n_{\rm gas}~{\rm [10^{4}~cm^{-3}]}$",r"$\chi~{\rm [Habing]}$"],
                    truths=truths, truth_color="#FFA833", show_titles=True,quantiles=(0.16, 0.5, 0.84), label_kwargs={"fontsize": 12}, labelpad=-.1)


fig.savefig("RCW79A-corner-non.pdf",bbox_inches='tight',pad_inches=0)
